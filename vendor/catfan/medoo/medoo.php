...
class medoo
{
		...
		if (is_array($where))
		{
			...
			if (isset($where[ 'ORDER' ]))
			{
				...
				else
				{
                    ...
                    $ORDER=@$ORDERs[0];
					$where_clause .= ' ORDER BY ' . $this->column_quote($ORDER).' '.@$ORDERs[1];
				}
			}
            if(!empty($this->page)){
                $where_clause .= ' LIMIT ' . $this->page[0] . ',' . $this->page[1];
                $this->page=[];
            }
            else {
    ...
    public function selects($tables,$columns = null, $where = null,$is_find=false){
        $table='';
        $column='';
        ...
        if($query){
            ...
            if($is_find){
                if(is_array($columns) or $columns=='*'){
                    ...
                    $leng=strlen(substr($column,0,strripos($column,'.')+1));
                    return $data[0][substr($column,$leng,strlen($column))];
                }
            }
            else{
                return $data;
