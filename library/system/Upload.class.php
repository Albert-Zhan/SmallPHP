...
class Upload {
    ...
    public function __construct($config = array(), $driver = '', $driverConfig = null){
        ...
        if($this->config['driver']=='Local') {
            if (!is_dir($this->config['rootPath'])) {
                $this->config['rootPath'] = APP_PATH . $this->config['rootPath'];
            }
        }
        /* 设置上传驱动 */
    ...
    public function upload($files='') {
        ...
        /* 检测上传根目录 */
        if(!self::$uploader->checkRootPath($this->config['rootPath'])){
            $this->error = self::$uploader->getError();
            return false;
        }

        /* 检查上传目录 */
        if(!self::$uploader->checkSavePath($this->config['savePath'])){
            $this->error = self::$uploader->getError();
            return false;
        }
        ...
        foreach ($files as $key => $file) {
            ...
            /* 保存文件 并记录保存成功的文件 */
            if (self::$uploader->save($file,$this->config['replace'])) {
    ...
    private function setDriver($driver = null, $config = null){
        $driver = $driver ? : ($this->config['driver']       ? : 'Local');
        $config = $config ? : ($this->config['driverConfig'] ? : '');
        $class = strpos($driver,'\\')? $driver : '\system\\Upload\\driver\\'.ucfirst(strtolower($driver));
    ...
    private function getSubPath($filename) {
        ...
        if ($this->config['autoSub'] && !empty($rule)) {
            ...
            if(!empty($subpath) && !self::$uploader->mkdir($this->config['savePath'] . $subpath)){
                $this->error = self::$uploader->getError();
                return false;
            }
        }
