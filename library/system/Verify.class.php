...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
        $codes=\system\Session::get($key,'Verify');
        if(!$codes){
            return false;
        }
        if($this->authcode(strtoupper($code)) == $codes) {
            $this->config['reset'] && \system\Session::del($key,'Verify');
            return true;
        }
        return false;
    }
    ...
    public function entry($id = '') {
        ...
        if($this->config['useZh']){ // 中文验证码
            for ($i = 0; $i<$this->config['length']; $i++) {
                ...
                imagettftext($this->_image, $this->config['fontSize'], mt_rand(-40, 40), $this->config['fontSize']*($i+1)*1.5, $this->config['fontSize'] + mt_rand(10, 20), $this->_color, $this->config['fontttf'], $code[$i]);
            }
        }else{
            for ($i = 0; $i<$this->config['length']; $i++) {
        ...
        \system\Session::set($key.$id,$code,'Verify',$this->config['expire']);
