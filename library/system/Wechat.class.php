...
class Wechat{
    ...
    public function jsapi_ticket(){
        ...
        if($cache->has('jsapi_ticket')){
            ...
            $res = $this->https_request($url);
            $result = json_decode($res, true);
            $cache->set('jsapi_ticket',$result["ticket"],7200);
     ...
     * 微信正式号获取openid
     * @param $redurl 跳转url
     * @param array $options 回调时传入的参数
     * @return string $info
     */
    public function snsapi_base($redurl,$options=array()){
        $options=empty($options)?'&state=123':http_build_query($options);
        if (!$_GET['code']) {
            $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$this->appid."&redirect_uri=".$redurl."&response_type=code&scope=snsapi_base&".$options."#wechat_redirect";
            header('location:'.$url);
            exit;
        }
        else{
            $info=$this->getUseropenID();
            return $info['openid'];
        }
    }

    /**
     * 微信正式号获取openid和用户基本信息
     * @param $redurl 跳转url
     * @param array 回调时传入的参数
     * @return array
     */
    public function snsapi_userinfo($redurl,$options=array()){
        if (!$_GET['code']) {
            $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$this->appid."&redirect_uri=".$redurl."&response_type=code&scope=snsapi_userinfo&state=".$options."#wechat_redirect";
            header('location:'.$url);
            exit;
        }
        else{
    ...
    public function getUseropenID(){
        ...
        $res = $this->https_request($url,$data=null);
        return $res;
    }
    ...
    //微信测试号获取Openid
    public function debug_getOpenid(){
        $postStr = $GLOBALS["HTTP_RAW_POST_DATA"];
        if (!empty($postStr)){
            $this->logger("R \r\n".$postStr);
            $postObj = simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);  
        ...
        switch ($object->Event)
        {
            ...
            case "CLICK":
                switch ($object->EventKey)
                {
                    case "COMPANY":
                        $content = array();
                        $content[] = array("Title"=>"Z-PHP", "Description"=>"Z-PHP框架", "PicUrl"=>"", "Url" =>"http://zphp.5lazy.cn");
                        break;
                    default:
    ...
    private function receiveText($object)
    {
        ...
        //多客服人工回复模式
        if (strstr($keyword, "请问在吗") || strstr($keyword, "在线客服")||strstr($keyword, "客服")){
